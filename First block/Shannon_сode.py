...
def task(N, line, c):
    ...
    # таблица
    mytable = PrettyTable()
    mytable.field_names = ["i", "Xi", "Pi", "qi", "I(xi)", "li", "Ci"]
    ...
    for i in range(len(array_PxN1)):
        for j in range(len(array_PxN1)-1):
            if array_PxN1[j] < array_PxN1[j+1]:
                array_PxN1[j], array_PxN1[j+1] = array_PxN1[j+1], array_PxN1[j]
                array_Char1[j], array_Char1[j+1] = array_Char1[j+1], array_Char1[j]
    ...
    for i in array_PxN1:
        array_Li.append(ceil(abs(log2(i))))
    ...
    # формируем таблицу
    for i in range(len(array_PxN1)):
        mytable.add_row([i+1, array_Char1[i], round(array_PxN1[i], 4), array_Qi[i], round(abs(log2(array_PxN1[i])),4), array_Li[i], array_Ci[i]])
